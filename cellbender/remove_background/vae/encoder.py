...
class EncodeNonZLatents(nn.Module):
    ...
    def __init__(self,
        ...
        # Set up the linear transformations used in fully-connected layers.
        # Adjust initialization conditions to start with a reasonable output.
        self.linears = nn.ModuleList([nn.Linear(3 + self.n_genes, # + self.z_dim,
                                                hidden_dims[0])])
        with torch.no_grad():
    ...
    def forward(self,
        ...
        # print(f'overlap[counts < 1000].mean() is {overlap[counts < 1000].mean()}')
        # z = self._pca(z.cpu()).to(x.device)
        # print(f'z.shape is {z.shape}')
        # print(f'z[counts > 1000].abs().mean() is {z[(counts > 1000).squeeze(), :].abs().mean()}')
        # print(f'z[counts < 1000].abs().mean() is {z[(counts < 1000).squeeze(), :].abs().mean()}')
        ...
        if self.x_scaling is None:
            ...
            for i in range(n_std_est):
                ...
                # std_estimates[i] = z.view(-1)[idx][:num].std().item()
            ...
            # print(f'std is {z.std().item()} and robust_std is {robust_std}')
            # print(f'scaled z has mean {(z * self.x_scaling).mean()} and std {(z * self.x_scaling).std()}')
